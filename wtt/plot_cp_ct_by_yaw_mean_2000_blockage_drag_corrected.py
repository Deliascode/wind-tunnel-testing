...
OUTPUT_DIR = PROJECT_ROOT / "plots_cp_ct_mean_2000_blockage_drag_corrected"
...
def interpolate_to_target_length(
    ...
    source_time = np.linspace(0.0, duration_s, values.size, endpoint=False)
    target_time = np.linspace(0.0, duration_s, target_length, endpoint=False)

    return np.interp(target_time, source_time[finite_mask], values[finite_mask])
...
def axial_induction_from_ct(ct: np.ndarray) -> np.ndarray:
    ...
    Values are clipped to the classical range to avoid complex values.
    """
...
def glauert_velocity_from_thrust(
    ...
    ct_denominator_uncorrected = 0.5 * air_density * ROTOR_AREA_M2 * u_inf**2
...
def drag_corrected_rotor_thrust_and_velocity(
    ...
    for _ in range(CORRECTION_MAX_ITERATIONS):
        ...
        ct_corrected_denominator = (
            0.5 * air_density * ROTOR_AREA_M2 * u_blockage_corrected**2
        )
        ...
        nacelle_drag_n = (
            0.5 * air_density * u_blockage_corrected**2 * NACELLE_HUB_SCD_M2
        )
        nacelle_drag_moment_nm = nacelle_drag_n * NACELLE_HUB_MOMENT_ARM_M
...
def compute_test_statistics(mat_path: Path) -> dict:
    """Computes mean TSR, cP and cT on a 2000-sample time axis."""
    ...
    thrust_raw_from_tower_fa = -tower_fa_common / THRUST_LEVER_ARM_M
    ...
    cp_denominator = 0.5 * air_density_common * ROTOR_AREA_M2 * u_blockage_corrected**3
    cp_series = np.divide(
        power_w,
        cp_denominator,
        out=np.full(TARGET_LENGTH, np.nan, dtype=float),
    ...
    ct_denominator_corrected = (
        0.5 * air_density_common * ROTOR_AREA_M2 * u_blockage_corrected**2
    )
    ct_series = np.divide(
        thrust_n,
        ct_denominator_corrected,
    ...
    # Representative mean TSR:
    # mean blade-tip speed divided by mean blockage-corrected velocity.
    tip_speed_mean = safe_stat(tip_speed, np.mean)
    u_blockage_corrected_mean = safe_stat(u_blockage_corrected, np.mean)

    if (
        np.isfinite(tip_speed_mean)
        and np.isfinite(u_blockage_corrected_mean)
        and u_blockage_corrected_mean != 0.0
    ):
        tsr_mean = tip_speed_mean / u_blockage_corrected_mean
    else:
        tsr_mean = math.nan

    return {
        "yaw_actual_mean": safe_stat(yaw_common, np.mean),
        "pitot_velocity_mean": safe_stat(pitot_velocity_common, np.mean),
        "u_blockage_corrected_mean": u_blockage_corrected_mean,
        "ct_for_blockage_mean": safe_stat(correction["ct_for_blockage"], np.mean),
        "blockage_factor_mean": safe_stat(correction["blockage_factor"], np.mean),
        "thrust_raw_from_tower_fa_mean": safe_stat(thrust_raw_from_tower_fa, np.mean),
        "rotor_thrust_drag_corrected_mean": safe_stat(thrust_n, np.mean),
        "nacelle_drag_mean": safe_stat(correction["nacelle_drag_n"], np.mean),
        "nacelle_drag_moment_mean": safe_stat(
            correction["nacelle_drag_moment_nm"], np.mean
        ),
        "tower_drag_mean": safe_stat(correction["tower_drag_n"], np.mean),
        "tower_drag_moment_mean": safe_stat(
            correction["tower_drag_moment_nm"], np.mean
        ),
        "tower_velocity_mean": safe_stat(correction["tower_velocity"], np.mean),
        "axial_induction_mean": safe_stat(correction["axial_induction"], np.mean),
        "tip_speed_mean": tip_speed_mean,
        "tsr_mean": tsr_mean,
        ...
        rows.append(
            {
                ...
                "tip_speed_mean": stats["tip_speed_mean"],
                "pitot_velocity_mean": stats["pitot_velocity_mean"],
                "u_blockage_corrected_mean": stats["u_blockage_corrected_mean"],
                "ct_for_blockage_mean": stats["ct_for_blockage_mean"],
                "blockage_factor_mean": stats["blockage_factor_mean"],
                "thrust_raw_from_tower_fa_mean": stats[
                    "thrust_raw_from_tower_fa_mean"
                ],
                "rotor_thrust_drag_corrected_mean": stats[
                    "rotor_thrust_drag_corrected_mean"
                ],
                "nacelle_drag_mean": stats["nacelle_drag_mean"],
                "nacelle_drag_moment_mean": stats["nacelle_drag_moment_mean"],
                "tower_drag_mean": stats["tower_drag_mean"],
                "tower_drag_moment_mean": stats["tower_drag_moment_mean"],
                "tower_velocity_mean": stats["tower_velocity_mean"],
                "axial_induction_mean": stats["axial_induction_mean"],
...
def plot_for_yaw(rows: list[dict], yaw_value: float, coefficient_prefix: str):
    ...
    Produces one plot with: mean, mean+std, mean-std, min, max.
    """

    yaw_rows = [r for r in rows if r["yaw_target"] == yaw_value]
    yaw_rows.sort(
        key=lambda r: (r["tsr_mean"] if r["tsr_mean"] is not None else math.inf)
    )
    ...
    plt.plot(x, y_plus_std, marker="o", linestyle="--", label="Mean + standard deviation")
    plt.plot(x, y_minus_std, marker="o", linestyle="--", label="Mean - standard deviation")
    ...
    plt.title(f"{coeff_label} over TSR at yaw = {yaw_value}°")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    out_name = f"{coeff_label}_mean_yaw_{int(yaw_value):+d}.png"
    out_name = out_name.replace("+", "plus").replace("-", "minus")
...
def save_summary_csv(rows: list[dict]):
    ...
    fieldnames = [
        ...
        "tip_speed_mean",
        "pitot_velocity_mean",
        "u_blockage_corrected_mean",
        "ct_for_blockage_mean",
        "blockage_factor_mean",
        "thrust_raw_from_tower_fa_mean",
        "rotor_thrust_drag_corrected_mean",
        "nacelle_drag_mean",
        "nacelle_drag_moment_mean",
        "tower_drag_mean",
        "tower_drag_moment_mean",
        "tower_velocity_mean",
        "axial_induction_mean",
        "cp_mean",
        "cp_plus_std",
        "cp_minus_std",
        "cp_min",
...
def main():
    ...
    print("PitotVelocity wird für die Glauert-Korrektur verwendet.")
    print("Danach werden TSR, cP und cT mit u_blockage_corrected berechnet.")
    print("Für die Kurven wird der arithmetische Mittelwert verwendet.")
    ...
    print(f"Windkanalfläche: {TUNNEL_CROSS_SECTION_M2:.3f} m²")
