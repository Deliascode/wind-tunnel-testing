...
OUTPUT_DIR = PROJECT_ROOT / "plots_cp_ct_welch_grouped_by_yaw_zero_glauert_factor"
...
def compute_derived(
    ...
    # The Glauert correction is calculated from the matching 0-degree case.
    # For every yaw case, apply that same correction factor to the velocity
    # measured in the current MAT file. This does NOT reuse the 0-degree
    # corrected velocity itself.
    blockage_factor = zero_ref["blockage_factor"]
    u_glauert = measured["pitot_velocity"] * blockage_factor
...
def save_summary_csv(rows: list[dict]) -> Path:
    ...
    path = OUTPUT_DIR / "cp_ct_welch_summary_grouped_by_yaw_zero_glauert_factor.csv"
...
def save_arrays_npz(yaw_cases: dict[str, dict]) -> Path:
    path = OUTPUT_DIR / "yaw_grouped_time_series_arrays_welch_zero_glauert_factor.npz"
    arrays = {}
...
def main():
    ...
    # Glauert correction factors are calculated once from the 0-degree yaw cases.
    zero_refs = build_zero_yaw_references(grouped)

    # Yaw cases remain separated. Each case applies the matching zero-yaw
    # Glauert correction factor to its own measured Pitot velocity.
    ...
    print("Glauert correction factors are calculated only from the matching 0-degree yaw case.")
    print("Those factors are applied to each yaw case's own measured Pitot velocity.")
    print("C_T for Glauert uses zero-yaw tunnel/Pitot velocity, not Glauert-corrected velocity.")
